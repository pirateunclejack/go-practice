...
func AddAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Printf("Failed to get object if from user id hex: %v", err)
		...
		if err!= nil {
			log.Printf("failed to get aggregate from user collection: %v", err)
			c.IndentedJSON(500, "internal server error")
			return
		}
		...
		if size < 2 {
			...
			c.JSON(http.StatusOK, "add address successfully")
		} else {
...
func ListAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := UserCollection.FindOne(
			ctx, bson.D{primitive.E{Key: "_id", Value: usert_id}}).Decode(&user)
		if err != nil {
			log.Printf("failed to get user when listing address: %v", err)
			c.IndentedJSON(500, "failed to get user when listing address")
			return
		}
